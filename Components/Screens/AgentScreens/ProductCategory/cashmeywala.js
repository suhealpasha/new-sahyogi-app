...
class Cashmeywala extends Component {
  constructor(props) {
    super(props);
    this.state = {
      ...
      checked: false,
      data:  [
        {id:1,checked:false,name: 'Sun Glass', icon: 'sunGlass'},
        {id:2,checked:false,name: 'Frames', icon: 'frames'},
        {id:3,checked:false,name: 'Contact Lens', icon: 'cLens'},
        {id:4,checked:false,name: 'Spectacle Lens', icon: 'sLens'},
        {id:5,checked:false,name: 'Eye Solution', icon: 'eSolution'},
      ],
    ...
    return (
      ...
      <View style={{flex: 1.0, backgroundColor: 'fafcff'}}>       
        ...
        <ImageBackground style={{
         ...
         resizeMode='cover' source={require('../../../../assets/Images/banner/banner3.png')}/>
       
          </View> 
          <View style={styles.container}>
          <FlatList
            data={this.state.data}
            columnWrapperStyle={{allignItems: 'space-between',justifyContent:'space-between'}}
            numColumns={3}
                ...
                <TouchableNativeFeedback
                    ...
                    >
                     ...
                     height:20,display:'flex',alignItems:'flex-end'
                       ,width:'100%',paddingRight:5,paddingTop:5}}> 
                    {item.checked ? <RoundCheckbox
                      ...
                      <View
                        ...
                        >
                  {item.icon === 'sunGlass' ? <SunGlassIcon /> : null}
                      {item.icon ==='frames' ? <FramesIcon /> : null}
                      {item.icon === 'cLens' ? <EyelensIcon /> : null}
                      {item.icon === 'sLens' ? <SpectacleLensIcon /> : null}
                      {item.icon === 'eSolution' ? <EyeSolutionsIcon /> : null}
                      </View>
                      <View>
